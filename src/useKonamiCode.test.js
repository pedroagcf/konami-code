...
describe("useKonamiCode", () => {
	...
	it("should reset the sequence if the user types a wrong sequence", () => {
		const { result } = renderHook(() => useKonamiCode());
		act(() => {
			window.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrownUp" }))
	...
	it("should allow diferente sequence and return if user types the sequence", () => {
		const newSequence = ["j", "k"]
